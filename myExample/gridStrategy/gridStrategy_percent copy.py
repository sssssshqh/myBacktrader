...
def getBestStepPercent(lowLimitPrice, highLimitPrice, 
    ...
    strageParams = {'initCash': initCash, 'cashUsageRate': cashUsageRate, 'lowLimitPrice': lowLimitPrice, 'highLimitPrice': highLimitPrice, 'benchmarkPrice': '1'}
    grids = []
    ...
    # Add a strategy
    if(isPrint):
        cerebro.addstrategy(TestStrategy, grid=grids[0], isPrint=isPrint)
    else:
        cerebro.optstrategy(TestStrategy, grid=grids, isPrint=isPrint)
...
calculateGrid(isPrint=False, isDownload=False)